...
describe("The `generate` command", () => {
    ...
    it("throws an error if you don't pass any args", async () => {
        const generateCommand = () => child_process_1.execSync(`./bin/run generate`, {
            cwd: root
        });
    ...
    it("works with a arg of a valid template and uses the default src/components path flag", async () => {
        ...
        child_process_1.execSync(`./bin/run generate react-component -n ${componentName}`, {
            cwd: root
        });
        const newComponentFolderExists = await fse.pathExists(componentFolderPath);
        const componentIndexExists = fse.existsSync(`${componentFolderPath}/index.js`);
    ...
    it("works with the react-typescript template and uses the default src/components path flag", async () => {
        ...
        const componentFolderPath = `${tempRoot}/${componentName}`;
        child_process_1.execSync(`./bin/run generate react-typescript-component -n ${componentName}`, {
            cwd: root
        });
        const newComponentFolderExists = await fse.pathExists(componentFolderPath);
        const componentIndexExists = fse.existsSync(`${componentFolderPath}/index.ts`);
        const componentTsxExists = fse.existsSync(`${componentFolderPath}/${componentName}.tsx`);
        expect(newComponentFolderExists).toBe(true);
        expect(componentIndexExists).toBe(true);
    ...
    it("works with the react-native-typescript-component template and uses the default src/components path flag", async () => {
        ...
        const componentFolderPath = `${tempRoot}/${componentName}`;
        child_process_1.execSync(`./bin/run generate react-native-typescript-component -n ${componentName}`, {
            cwd: root
        });
        const newComponentFolderExists = await fse.pathExists(componentFolderPath);
        const componentIndexExists = fse.existsSync(`${componentFolderPath}/index.ts`);
        ...
        const hasStoryBookDir = fse.existsSync(`${tempRoot}/storybook/stories/index.ts`);
        // If they do,
        // we assert that a .stories.tsx file was generated
        if (hasStoryBookDir) {
            const componentStoriesExists = fse.existsSync(`${componentFolderPath}/${componentName}.stories.tsx`);
    ...
    it("works with the react-native-e2e template and uses the default e2e path", async () => {
        ...
        const e2ePath = "e2e";
        child_process_1.execSync(`./bin/run generate react-native-e2e -n ${testName}`, {
            cwd: root
        });
    ...
    it("works with the util-typescript template ", async () => {
        ...
        child_process_1.execSync(`./bin/run generate util-typescript -n ${testName} -p ${utilsPath}`, {
            cwd: root
        });
    ...
    it("uses the path flag if passed", async () => {
        ...
        child_process_1.execSync(`./bin/run generate react-component -n ${componentName} -p ${customPath}`, {
            cwd: root
        });
        const newComponentFolderExists = await fse.pathExists(componentFolderPath);
        const componentIndexExists = fse.existsSync(`${componentFolderPath}/index.js`);
    ...
    it("throws an error when you pass an invalid flag", () => {
        const generateCommand = () => child_process_1.execSync(`./bin/run generate fake-component -n FakeComponent`, {
            cwd: root
        });
        expect(generateCommand).toThrowErrorMatchingSnapshot();
    });
    it("uses the users template if they have one", async () => {
        ...
        await fse.copyFileSync(`${root}/src/__tests__/helpers/componentTestTemplate.ejs.t`, `${pathToNewTemplate}/component.ejs.t`);
        const newTemplateFolderExists = await fse.pathExists(pathToNewTemplate);
        const newTemplateFileExists = fse.existsSync(`${pathToNewTemplate}/component.ejs.t`);
        ...
        child_process_1.execSync(`./bin/run generate react-component -n ${newComponentName}`, {
            cwd: root
        });
        // Using the template we created, we expect to see a new component
        const newGeneratedComponentExists = fse.existsSync(`${tempRoot}/${newComponentName}.jsx`);
        // And no folder for the component
        const newGeneratedComponentFolderExists = await fse.pathExists(`${tempRoot}/${newComponentName}`);
